...
<jsp:include page="../includes/header.jsp"></jsp:include>
    ...
    <form action="boardUpdate.do" method="GET">
        <table class="table">
            ...
            <tbody>
                <input type="text" hidden name="<%= vo.getBoardNo() %>">
                <tr class="text-center">
                    <th>제묙</th>
                    <td colspan="2" class="text-start"><%= vo.getTitle() %></td>
                </tr>
                <tr >
                    <th colspan="2" class="text-center">내용</th>
                    <td colspan="2" class="text-start">
                        <textarea name="" id="" cols="100%" rows="10" class="p-2" readonly><%= vo.getContent() %></textarea>
                    </td>
                </tr>
                <tr class="text-center">
                    <th colspan="2" class="text-center">작성자</th>
                    <td colspan="2" class="text-start"><%= vo.getMemberId() %></td>
                </tr>
                <tr class="text-center">
                    <th colspan="2" class="text-center">작성일시</th>
                    <td colspan="2" class="text-start"><%= wdate %></td>
                </tr>
            </tbody>
        </table>
    ...
    buttonEle.forEach((ele) => {
        ele.addEventListener('click', (event) => {
            if(event.target.textContent == "수정") {
                location.href = 'boardUpdate.do?bno=<%= vo.getBoardNo() %>';
            } 
            if(event.target.textContent == "삭제") {
                location.href = 'boardDelete.do?bno=<%= vo.getBoardNo() %>';
            } 
        });
    });
